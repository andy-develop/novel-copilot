---
name: novel-copilot
description: "长篇小说全自动写作助手 v4.0：给一个开头自动成书。发散→评分→选择→写作→校验，循环不停。23大模块：逻辑一致性 + 14种发散 + 一致性门控 + 叙事纪律 + 读者情绪 + 爽度引擎 + 画面感引擎 + 快节奏 + 散文质量层 + 章节解剖引擎 + 伏笔自动发现 + 质量自动评分 + A/B分歧 + 预测颠覆 + 角色深度 + 情感波动曲线 + 对话潜台词 + 章节变速 + 预加载缓存 + 批量发散 + Delta更新。30万字不崩盘，每章都有爽点和画面。"
version: 4.0.0
author: Hermes Agent
